...
The final dataset contains raw files, metadata, and `.mat` files along with the reformatted `.h5` files. <!-- TODO: add public download link for the released dataset. -->
...
You are advised to run the following unit tests after reformatting the dataset. They perform simple checks within the new data format and compare label/event lengths. Each test takes the dataset root directory as its last argument:

```
python test_suites/h5_data_counter.py <root_dir_to_new_dataset>
python test_suites/h5_event_label_comparison.py <root_dir_to_new_dataset>
```
...
If you have multiple recordings with multiple cameras to run, I recommend creating a simple bash file that loops over them, like the `run_pro_player_visualization.sh` example, and let it run.

```
bash run_pro_player_visualization.sh
```
...
- TODO
=======
The raw APS recordings (rosbags, segmented rallies, pre-processed ball positions) and the raw EVS recordings for the matches are part of the released dataset. <!-- TODO: add public download link(s) and access instructions for the released dataset. -->
>>>>>>> Stashed changes
