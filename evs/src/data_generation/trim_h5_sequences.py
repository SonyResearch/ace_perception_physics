...
from tools.h5_writer import H5Writer, H5WriterLabel
...
def find_filenames_by_sequence(directory, sequence_number):
    ...
    sequence_formatted = (
        f"seq_{int(sequence_number):03d}"  # Pads the sequence number with zeros
    )
...
def trim_h5(old_data_root: str, new_data_root: str, trim_csv_path: str):
    ...
    recording_names, camera_names, sequence_numbers, trim_starts, trim_ends = (
        read_csv_files(trim_csv_path)
    )
    ...
    for i, rec_name in enumerate(recording_names):
        ...
        old_h5_path, old_h5_label_path = find_filenames_by_sequence(
            seq_root, sequence_numbers[i]
        )
        ...
        events_class = Events(
            new_events["x"], new_events["y"], new_events["p"], new_events["t"]
        )
        ...
        for key_name in old_h5_label.keys():
            if end_trim_ms == -1:
                ...
                new_label_dict[key_name] = old_h5_label[key_name][
                    start_trim_ms:end_trim_ms
                ]

        # Initialize the new h5 label writer
        new_h5_label_path = new_h5_folder / Path(old_h5_label_path).name
        h5_writer_label = H5WriterLabel(
            new_h5_label_path, default_shape=new_label_dict[key_name].shape[0]
        )
