...
root_dir_path = pathlib.Path(__file__).parent.parent.parent.parent.parent.parent.parent.parent.absolute()
...
class ImageReconstructor:
    """
    Reconstructs images from events, given a trained RNN.
    """
    ...
    def initialize(self, height, width, options):
        "Initializes EventPreprocessor, Filters, and ImageWriters."
        ...
        self.perform_color_reconstruction = (
            options.color
        )  # whether to perform color reconstruction (only use this with the DAVIS346color)
        ...
        if self.perform_color_reconstruction:
            self.crop_halfres = CropParameters(int(width / 2), int(height / 2), self.model.num_encoders)
    ...
    def update_reconstruction(self, event_tensor, event_tensor_id, save_flag, period, stamp=None):
        "Get next frame by feeding events into the model."
        with torch.no_grad():
                ...
                for channel in events_for_each_channel.keys():
                    with CudaTimer("Inference"):
                        new_predicted_frame, states = self.model(
                            events_for_each_channel[channel], self.last_states_for_each_channel[channel]
                        )
                    ...
                    with CudaTimer("Tensor (GPU) -> NumPy (CPU)"):
                        reconstructions_for_each_channel[channel] = (
                            new_predicted_frame[0, 0, crop.iy0 : crop.iy1, crop.ix0 : crop.ix1].cpu().numpy()
                        )
            ...
            if save_flag:
                self.image_writer(out, event_tensor_id, period, stamp, events=events)
